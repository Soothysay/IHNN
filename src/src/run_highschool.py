...
from model_highschool import IGNN,final_predictor
from normalization import aug_normalized_adjacency
from sklearn.model_selection import train_test_split
hypergraph1 = []
file_path='../data/contact-high-school/hyperedges-contact-high-school.txt'#'../data/contact-high-school/hyperedges-contact-high-school.txt'
...
#with open('../data/contact-high-school/node-labels-contact-high-school.txt', 'rb') as handle:
    #labels = pickle.load(handle)
labels1=[]
with open('../data/contact-high-school/node-labels-contact-high-school.txt', 'r') as file:
    for line in file:
...
train, test = train_test_split(overall, test_size=0.7, random_state=2)
...
#edge_feat=[]
edge_feat=torch.load('../data/output_submit/school_edge.pt')
...
edge_loss = node_in_edge_loss(subgraph_list,embedding_dim=128, hidden_dim=128).to('cuda:2')
#model_pred=final_predictor(nfeat=64,head=4,nclass=len(classes)).to('cuda:2')
...
for ep in range(epochs):
    ...
    if testy<acc_test:
        ...
        print('Test acc')
        print(testy)
        torch.save(output[test],'../data/outputs/ihnn_output121.pt')
        torch.save(labels_torch[test],'../data/outputs/labels_ihnn121.pt')
...
plt.savefig('plots/school/training_loss_seed_9.png')
plt.close()
...
plt.savefig('plots/school/acc.png')
plt.close()
