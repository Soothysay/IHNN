...
from model_v_2_0 import IGNN,final_predictor
from normalization import aug_normalized_adjacency
with open('../data/coauthorship/dblp/hypergraph.pickle', 'rb') as handle:
    hypergraph = pickle.load(handle)
with open('../data/coauthorship/dblp/features.pickle', 'rb') as handle:
    features = pickle.load(handle).todense()
with open('../data/coauthorship/dblp/labels.pickle', 'rb') as handle:
    labels = pickle.load(handle)
with open('../data/coauthorship/dblp/splits/9.pickle', 'rb') as handle:
...
labels_torch=torch.from_numpy(np.array(labels)).to('cuda:1')
#labels_torch=torch.from_numpy(np.array(labels)).to('cuda:1')
my_hypergraph=Hypergraph(num_v=len(labels),e_list=list(hypergraph.values()),device='cuda:0')
#features_torch_node=torch.from_numpy(features).to('cuda:1')
edge_feat=[]
for e in my_hypergraph.e[0]:
    ef=[]
    for n in e:
        ef.append(features[n].tolist())
    #noormalize ef by dividing by len(e)
    ef=np.array(ef).sum(axis=0)/len(e)
    edge_feat.append(ef)
#import pdb;pdb.set_trace()
edge_feat=np.array(edge_feat,dtype=np.float32)
edge_feat=edge_feat.reshape(len(my_hypergraph.e[0]),features.shape[1])
#import pdb;pdb.set_trace()
#features_torch_edge=nn.Embedding(len(my_hypergraph.e[0]),features_torch_node.shape[1]).to('cuda:1')

#block=torch.cat([torch.cat([my_hypergraph.n2e,torch.zeros(len(my_hypergraph.e[0]),len(my_hypergraph.e[0])).to_sparse().to('cuda:0')], dim=1),torch.cat([torch.zeros(len(labels),len(labels)).to_sparse().to('cuda:0'),my_hypergraph.e2n], dim=1)], dim=0)
...
# edge_loss = hyperedge_loss(train_label_matrix)
edge_loss = node_in_edge_loss(subgraph_list).to('cuda:1')

adj1=aug_normalized_adjacency(block.to_dense().cpu())
adj=torch.sparse_coo_tensor(adj1.nonzero(), adj1.data, adj1.shape).to('cuda:1')
my_hypergraph=my_hypergraph.to('cpu')
torch.cuda.empty_cache()
features_torch=torch.cat([torch.from_numpy(features).to('cuda:1'),torch.from_numpy(edge_feat).to('cuda:1')],dim=0)
#features_torch=torch.cat([features_torch_edge.weight,features_torch_node],dim=0)
...
optimizer=optim.Adam((list(model.parameters()) + list(edge_loss.parameters())),
                     lr=0.01,weight_decay=5e-4) #0.0009   5e-4
...
for ep in range(epochs):
    ...
    optimizer.zero_grad()
    #import pdb;pdb.set_trace()
    output = model(features_torch, adj)
    ...
    node_logits = F.log_softmax(node_embeddings, dim=1)
...
plt.savefig('plots/dblp/training_loss_seed_9.png')
plt.close()
...
plt.savefig('plots/dblp/acc.png')
plt.close()